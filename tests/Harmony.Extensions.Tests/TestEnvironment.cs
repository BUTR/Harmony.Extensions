...
namespace HarmonyLibTests
{
    ...
    public class TestEnvironment : TestLogger
    {
        ...
        {
            return AccessTools2.Property(type, propertyName)?.GetValue(null, new object[0])?.ToString() ?? "null";
        }

        static string CallGetter(Type type, string methodName)
        {
            return AccessTools2.Method(type, methodName)?.Invoke(null, new object[0])?.ToString() ?? "null";
        }
